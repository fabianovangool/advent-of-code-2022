...
const parsedInstructions = dayFiveInput.split(/\n/).slice(11).map((line) => line.replace("move ", "").replace(" from ", ",").replace(" to ", ",").split(/,/).map((element) => Number(element)));

const executeAllInstructions = (part) => {
	const containers = parsedContainers;
	...
	const movedContainers = part === "one" ?
		containers[from].slice(containers[from].length - amount).reverse() :
		containers[from].slice(containers[from].length - amount);


	containers[to].push(...movedContainers);
	Array(amount).fill(0, 0).map((_) => containers[from].pop());
	});
